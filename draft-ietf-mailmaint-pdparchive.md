...
docname: draft-ietf-mailmaint-pdparchive-latest
submissiontype: IETF
number:
date:
consensus: true
...
venue:
  ...
  latest: "https://lisad.github.io/draft-happel-mailmaint-pdparchive/draft-ietf-mailmaint-pdparchive.html"
...
The archive.json file consists of three main sections with metadata about the archive as exported, the dataset used, and the source of the data.
...
~~~~~~~~~~
{::include ./schemas/archive-schema.json}
~~~~~~~~~~
{: #archive-schema title="General JSON Schema for archive.json"}



Example of archive.json (full export):

~~~~~~~~~~
{::include ./examples/archive.json}
~~~~~~~~~~
...
| uid | string | SHOULD* | Use OBJECTID from {{RFC8474}}
...
UIDs SHOULD always be strings.  This allows systems with UIDs that are not
...
Finally, to show an example containing no IMAP content but also meets schema
requirements:

~~~
{::include ./examples/folder-tasks.json}
~~~
{: #example-folder-tasks title="A folder.json example with incremental changes"}
...
VTIMEZONE objects are not part of this archive format.  Timezones are more
...
data, this specification can be extended to explain how that would work.
